            ...
            <Form onSubmit={this.handleSubmit}>
              <FormItem className={styles.form_item}>
                {getFieldDecorator("mobile", {
                  rules: [{ required: true, message: "请输入注册的手机号码" }, { pattern: /^1[34578]{1}\d{9}$/, message: "您输入的手机号有误" }]
              ...
              <FormItem className={styles.form_item}>
                {getFieldDecorator("password", {
                  rules: [{ required: true, message: "请输入旧密码" }, { pattern: /^(\w){6,14}$/, message: "请输入6-14位字符的密码" }]
                })(<Input className={styles.input} type="password" placeholder="请输入旧密码" />)}
              </FormItem>
              <FormItem className={styles.form_item}>
                {getFieldDecorator("newPassword", {
                  rules: [{ required: true, message: "请输入新密码" }, { pattern: /^(\w){6,14}$/, message: "请输入6-14位字符的密码" }]
